...
void BootStrap::loggerInit(const adbase::TimeZone& tz) {
	...
	if (_configure->isAsync) { // 只有是守护进程开启的时候日志开启异步写入
		...
		// 启动异步日志落地
		std::string basename = _configure->logsDir + std::string(::basename("aidt"));
...
void BootStrap::usage() {
	std::cout << "Usage: aidt [options...] <path>" << std::endl;
...
void BootStrap::parseOption(int argc, char **argv) {
	int ch;
	while((ch = getopt(argc, argv, "c:hvt")) != -1) {
		if (ch == 'c') {
			_configFile = optarg;
		} else if (ch == 'h') {
			usage();

...
adbase::mc::ProtocolBinaryResponseStatus McProcessor::set(const void* key,
    ...
    if (_context->app == nullptr || _context->message == nullptr) {
        LOG_ERROR << "System is starting.";
        return adbase::mc::PROTOCOL_BINARY_RESPONSE_NOT_STORED;
    }
	...
	std::vector<std::string> keys = adbase::explode(keyData, '#', true);
	if (keys.size() != 2 && keys.size() != 1) {
		LOG_TRACE << "Key: `" << keyData << "` is invalid.";
		return adbase::mc::PROTOCOL_BINARY_RESPONSE_AUTH_ERROR;	
	}

	int partId = 0;
	if (keys.size() == 2) {
		errno = 0;
		partId = static_cast<int>(strtoul(keys[1].c_str(), nullptr, 10));
		if (errno != 0) {
			LOG_TRACE << "Key partId: `" << keys[1] << "` is invalid.";
			return adbase::mc::PROTOCOL_BINARY_RESPONSE_AUTH_ERROR;	
		}
	} else {
		partId = -1;
    ...
    if (!_context->app->checkValidTopicAndPartition(keys[0], partId)) {
		LOG_TRACE << "Key topic or partId: `" << keyData << "` is invalid.";
		return adbase::mc::PROTOCOL_BINARY_RESPONSE_AUTH_ERROR;	
    }
	
	bool ret = _context->message->setMessage(keys[0], partId, bufferBody);
	if (!ret) {
		return adbase::mc::PROTOCOL_BINARY_RESPONSE_NOT_STORED;
	}
